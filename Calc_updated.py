...
def cut_metrics(G, S, d):
    ''' Computes actual cut strain, egdes crossing cut '''
    V = set(G.nodes)
    cut_edges = set()
    for u in S:
...
def calculate_possible_d(n):
    '''computes all possiblde degrees, given number of nodes'''
    possible_d = []
    for i in range(3, n):
        ganzzahlig = (n/(i+1)) % 1
        if ganzzahlig == 0:
...
def expected_cut_strain_exact(G, S, d, strain):
    '''Computes Expected Cut Strain as in Giakkoupis'''
    V = set(G.nodes)
    n = len(V)
    m = G.number_of_edges()
    delta = 1 / d
...
def calculate_expected_cut_strain_alternative(G, S, d, strain):
    '''Computes our Exp. Cut Strain alternatively'''
    V = set(G.nodes)
    n = len(V)
    m = G.number_of_edges()
    neighbors = {u: set(G.neighbors(u)) for u in G.nodes()}
        ...
        for a_prime in Na:  # number of iterations: d
            ...
            if (a_prime in (Na - {b} - Nb)) and (len(Nb - {a} - Na) != 0):
                ...
                for b_prime in (Nb - {a} - Na):
                    ...
                    if ((a in S) ^ (b in S)) or ((a_prime in S) ^ (b_prime in S)): # 16 cases
                        ...
                        # Calculate new values: (in 12 of the 16 cases the cut strain changes)
                        if (a in S) and (b not in S) and (a_prime in S) and (b_prime not in S): 
                        ...
                        elif (a in S) and (b not in S) and (a_prime not in S) and (b_prime in S):  
                            alpha_a = (len(Na.intersection(S)) + 1) / len(Na)
                            alpha_b = (len(Nb.intersection(S)) - 1) / len(Nb)
                            alpha_a_prime = (
                                len(Na_prime.intersection(S)) - 1) / len(Na_prime)
                            alpha_b_prime = (
                                len(Nb_prime.intersection(S)) + 1) / len(Nb_prime)
                        elif (a not in S) and (b in S) and (a_prime in S) and (b_prime not in S):  
                            alpha_a = (len(Na.intersection(S)) - 1) / len(Na)
                            alpha_b = (len(Nb.intersection(S)) + 1) / len(Nb)
                            alpha_a_prime = (
                                len(Na_prime.intersection(S)) + 1) / len(Na_prime)
                            alpha_b_prime = (
                                len(Nb_prime.intersection(S)) - 1) / len(Nb_prime)
                        elif (a not in S) and (b in S) and (a_prime not in S) and (b_prime in S):  
                            alpha_a = (len(Na.intersection(S)) + 1) / len(Na)
                            alpha_b = (len(Nb.intersection(S)) - 1) / len(Nb)
                            alpha_a_prime = (
                                len(Na_prime.intersection(S)) + 1) / len(Na_prime)
                            alpha_b_prime = (
                                len(Nb_prime.intersection(S)) - 1) / len(Nb_prime)
                        elif ((a not in S) and (b in S) and (a_prime not in S) and (b_prime not in S)) or ((a not in S) and (b in S) and (a_prime in S) and (b_prime in S)):  
                            alpha_a = (len(Na.intersection(S))) / len(Na)
                            alpha_b = (len(Nb.intersection(S))) / len(Nb)
                            alpha_a_prime = (
                                len(Na_prime.intersection(S)) + 1) / len(Na_prime)
                            alpha_b_prime = (
                                len(Nb_prime.intersection(S)) - 1) / len(Nb_prime)
                        elif ((a not in S) and (b not in S) and (a_prime in S) and (b_prime not in S)) or ((a in S) and (b in S) and (a_prime in S) and (b_prime not in S)):  
                            alpha_a = (len(Na.intersection(S)) - 1) / len(Na)
                            alpha_b = (len(Nb.intersection(S)) + 1) / len(Nb)
                            alpha_a_prime = (
                        ...
                        elif ((a not in S) and (b not in S) and (a_prime not in S) and (b_prime in S)) or ((a in S) and (b in S) and (a_prime not in S) and (b_prime in S)):  
                            alpha_a = (len(Na.intersection(S)) + 1) / len(Na)
                            alpha_b = (len(Nb.intersection(S)) - 1) / len(Nb)
                            alpha_a_prime = (
                                len(Na_prime.intersection(S))) / len(Na_prime)
                            alpha_b_prime = (
                                len(Nb_prime.intersection(S))) / len(Nb_prime)
                        elif ((a in S) and (b not in S) and (a_prime in S) and (b_prime in S)) or ((a in S) and (b not in S) and (a_prime not in S) and (b_prime not in S)):  
                            alpha_a = (len(Na.intersection(S))) / len(Na)
                            alpha_b = (len(Nb.intersection(S))) / len(Nb)
                            alpha_a_prime = (
                                len(Na_prime.intersection(S))-1) / len(Na_prime)
